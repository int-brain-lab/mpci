...
class MesoscopeTask(DynamicTask):
    ...
    def get_signatures(self, **kwargs):
        ...
        raw_imaging_folders = [
            p.relative_to(self.session_path).as_posix()
...
class MesoscopeRegisterSnapshots(MesoscopeTask, RegisterRawDataTask):
        ...
        signature = {
            "input_files": [
                I(
                    ...
                    f"{self.device_collection}/reference",
                    False,
                    register=True,
                ),
                I(
    ...
    def __init__(self, session_path, **kwargs):
        super().__init__(session_path, **kwargs)
        self.device_collection = self.get_device_collection(
            "mesoscope", kwargs.get("device_collection", "raw_imaging_data_??")
        )
    ...
    def _run(self):
        ...
        reference_images = list(
            chain.from_iterable(map(lambda x: x.find_files(self.session_path)[1], dsets))
        )
