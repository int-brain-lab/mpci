    ...
    def _run(self, remove_uncompressed=False, verify_output=True, overwrite=False, **kwargs):
        ...
        for (in_dir, infiles), out_id in zip(input_files, output_identifiers):
            ...
            if outfile.exists() and not overwrite:
                _logger.info(
                    "%s already exists; skipping...", outfile.relative_to(self.session_path)
                )
                outfiles.append(outfile)
            else:
                if not infiles:
                    _logger.info(
                        "No image files found in %s", in_dir.relative_to(self.session_path)
                    )
                    continue
                ...
                _logger.debug(
                    "Input files:\n\t%s",
                    "\n\t".join(
                        map(Path.as_posix, (x.relative_to(self.session_path) for x in infiles))
                    ),
                )
                ...
                cmd = 'tar -cjvf "{output}" "{input}"'.format(
                    output=outfile.relative_to(in_dir),
                    input='" "'.join(str(x.relative_to(in_dir)) for x in infiles),
                )
                _logger.debug(cmd)
                process = subprocess.Popen(
                    cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=in_dir
                )
                ...
                assert compressed_size > int(min_size), (
                    f"Compressed file < {min_size / 1024:.0f}KB"
                )
                _logger.info(
            ...
            if verify_output:
                ...
                compressed_files = set(
                    x.split()[-1] for x in filter(None, info.decode().split("\n"))
                )
