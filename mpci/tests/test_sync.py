...
class TestMesoscopeSyncTimeline(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.meta = {
            "scanImageParams": {
                "hRoiManager": {"linePeriod": self.line_period, "scanFrameRate": 13.6803}
            },
            "FOV": [],
        }
        nXnYnZ = [self.n_lines, self.n_lines, 1]
        for i, slice_id in enumerate(
            chain.from_iterable(map(lambda x: list(repeat(x, reps)), range(n_depths)))
        ):
...
class TestMesoscopeSync(IntegrationTestCase):
    ...
    required_files = [
        "mesoscope/test/2023-02-17/002",
        "mesoscope/test/2023-03-03/002",
        "mesoscope/SP061/2025-02-26/001",
    ]
    ...
    def test_single_depth(self):
        ...
        task = MesoscopeSync(
            self.session_path_0, device_collection="raw_imaging_data", one=self.one
        )
        status = task.run()
        assert status == 0
        ...
        expected = [0.0, 4.157940e-05, 8.315880e-05, 1.247382e-04, 1.663176e-04]
    ...
    def test_multiple_bouts(self):
        ...
        task = MesoscopeSync(
            self.session_path_1, device_collection="raw_imaging_data*", one=self.one
        )
        status = task.run()
        assert status == 0
        ...
        expected = [0.0, 4.157550e-05, 8.315100e-05, 1.247265e-04, 1.663020e-04]
    ...
    def test_multi_depth(self):
        ...
        kwargs = dict(
            save=False, sync=sync, chmap=chmap, device_collection=collections, events=events
        )
