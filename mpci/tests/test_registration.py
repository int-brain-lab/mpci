...
from mpci.chronic.registration.linalg import _nearest_neighbour_1d, surface_normal, find_triangle  # None in p2b
...
class TestMesoscopeFOVOutput(IntegrationTestCase):
    ...
    def test_mesoscope_fov(self):
        ...
        with mock.patch.object(task, 'register_fov') as mock_obj, \
...
class TestUpdateCraniotomyCenter(IntegrationTestCase):
    ...
    def setUp(self):
        ...
        with mock.patch.object(one, 'eid2path', return_value=self.session_path):
            self.task = MesoscopeFOVHistology(self.session_path, one=one, reference_session=ref_eid)
