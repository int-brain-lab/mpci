...
class MesoscopePreprocess(MesoscopeTask):
    ...
    def setUp(self, **kwargs):
        ...
        for file in files:
            ...
            process = subprocess.Popen(
                cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=file.parent
            )
            stdout, _ = process.communicate()  # b'x 2023-02-17_2_test_2P_00001_00001.tif\n'
            _logger.debug(stdout.decode())
            tifs = [
                file.parent.joinpath(x.split()[-1])
    ...
    def _masks2sparse(stat, ops):
        ...
        roi_mask_sp = sparse.COO(
            list(map(np.concatenate, coords)), np.concatenate(data), shape=shape
        )
        pil_mask_sp = sparse.COO(
            np.unravel_index(np.concatenate(pil_coords), shape), True, shape=shape
        )
    ...
    def _rename_outputs(self, suite2p_dir, frameQC_names, frameQC, rename_dict=None):
        ...
        for plane_dir in self._get_plane_paths(suite2p_dir):
            ...
            with ZipFile(
                fov_dir / "_suite2p_ROIData.raw.zip", "w", compression=ZIP_DEFLATED
            ) as zf:
            ...
            if frameQC is not None and len(frameQC) > 0:
                ...
                frameQC_names.to_csv(
                    fov_dir.joinpath("mpciFrameQC.names.tsv"), sep="\t", index=False
                )
            ...
            np.save(
                fov_dir.joinpath("mpci.badFrames.npy"), np.asarray(ops["badframes"], dtype=bool)
            )
            np.save(
                fov_dir.joinpath("mpciMeanImage.images.npy"),
                np.asarray(ops["meanImg"], dtype=float),
            )
            np.save(
                fov_dir.joinpath("mpciROIs.stackPos.npy"),
                np.asarray([(*s["med"], 0) for s in stat], dtype=int),
            )
            np.save(
                fov_dir.joinpath("mpciROIs.cellClassifier.npy"),
                np.asarray(iscell[:, 1], dtype=float),
            )
            np.save(
                fov_dir.joinpath("mpciROIs.mpciROITypes.npy"),
                np.asarray(iscell[:, 0], dtype=np.int16),
            )
            # clusters uuids
            ...
            (
                pd.DataFrame(
                    [(0, "no cell"), (1, "cell")], columns=["roi_values", "roi_labels"]
    ...
    def load_meta_files(self):
        ...
        meta_files = sorted(
            self.session_path.glob(f"{self.device_collection}/*rawImagingData.meta.*")
        )
        collections = sorted(set(f.parts[-2] for f in meta_files))
        # Check there is exactly 1 meta file per collection
        assert (
            len(meta_files)
            == len(list(self.session_path.glob(self.device_collection)))
            == len(collections)
        )
    ...
    def _consolidate_exptQC(exptQC):
        ...
        frame_qc_names = pd.DataFrame(
            list(enumerate(qc_labels)), columns=["qc_values", "qc_labels"]
        )
    ...
    def get_default_tau(self):
        ...
        match = next(
            filter(None, (re.match(r".+-(G\d[fms])$", g["allele"]) for g in genotype)), None
        )
    ...
    def _run(self, rename_files=True, use_badframes=True, **kwargs):
        ...
        if len(plane_folders) == 0 or overwrite:
            ...
            # Ingest tiff files
            try:
                plane_folders, _ = self.bin_per_plane(
                    metadata, save_folder=save_folder, save_path0=self.session_path, **kwargs
                )
            except Exception:
        ...
        if rename_files:
            ...
            out_files = chain.from_iterable(
                map(lambda x: x.find_files(self.session_path)[1], self.output_files)
            )
        else:
