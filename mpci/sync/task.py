...
class MesoscopeSync(MesoscopeTask):
    ...
    def _run(self, **kwargs):
        ...
        self.rawImagingData = alfio.load_object(
            self.session_path / next(iter(collections)), "rawImagingData"
        )
        ...
        legacy = kwargs.get(
            "legacy", False
        )  # this option may be removed in the future once fully tested
        mesosync = MesoscopeSyncTimeline(self.session_path, n_FOVs)
        _, out_files = mesosync.extract(
            save=True,
            sync=sync,
            chmap=chmap,
            device_collection=collections,
            events=events,
            use_volume_counter=legacy,
        )
        return out_files
